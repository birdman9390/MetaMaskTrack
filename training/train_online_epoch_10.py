...
from originalsgd import originalSGD
...
for i in range(1,anno.n_objects+1):
    ...
    if not alreadyTrained:
        ...
        trainloader[i] = DataLoader(db_train[i], batch_size=p['trainBatch'], shuffle=False, num_workers=0)
...
meta_dir = os.path.join(Path.save_offline_root_dir(), 'lr_' + str(0.0025) + '_wd_' + str(0.0001))
optimizer=dict()
...
#ccnt1=0
#ccnt10=0
#for name,module in nets[1].module.get_learnable_params().items():
#    print('name------------')
#    print(module)
#    print('------------')
...
for epochs in range(1,11):
    ...
    meta_alpha.clear()
    meta_alpha=torch.load(os.path.join(meta_dir,'meta_alpha_epoch-'+str(resume_epoch_parent)+'.pth'), map_location=lambda storage, loc: storage.cuda(0))
    ...
    for object_id in range(1, anno.n_objects+1):
        ...
        #optimizer[object_id]=onlineSGD([{'params':module,'lr':meta_alphas[epoch][name],'name':name} for name,module in nets[object_id].module.get_learnable_params().items()], momentum=0.9, weight_decay=weight_decay)

        optimizer[object_id]=onlineSGD([{'params':get_1x_lr_params_NOscale(nets[object_id]), 'lr':meta_alpha[epoch][0]}, {'params':get_10x_lr_params(nets[object_id]),'lr':meta_alpha[epoch][1]}],momentum=0.9,weight_decay=weight_decay)


        #optimizer[object_id] = originalSGD([{'params': get_1x_lr_params_NOscale(nets[object_id]), 'lr': base_lr}, {'params': get_10x_lr_params(nets[object_id]), 'lr': 10 * base_lr}], lr=base_lr, momentum=0.9, weight_decay=weight_decay)

        #cnt1=0
        #cnt10=0
        #print('-------------')
        #for name in get_1x_lr_params_NOscale(nets[object_id]):
        #    cnt1+=1
        #for name in get_10x_lr_params(nets[object_id]):
        #    cnt10+=1
        #print(cnt1)
        #print(cnt10)
        #print('--------------')

        #for name, module in nets[object_id].module.get_learnable_params().items():
        #    if module in get_1x_lr_params_NOscale(nets[object_id]):
        #        print('yes')
        #    elif module in get_10x_lr_params(nets[object_id]):
        #        print('yes2')
        #    else:
        #        print('no!!!')
        aveGrad = 0
        optimizer[object_id].zero_grad()
        ...
        for ii, sample_batched in enumerate(trainloader[object_id]):
            ...
            for df_id in deformations:
                ...
                input_rgb_mask = torch.stack([normalizer(input_rgb_mask[i]) for i in range(2)])
...
for ii in range(len(anno)):
    ...
    annot.append(final_output.data[0][0])
    ...
    final_pil_image.save(os.path.join(save_dir, seq_name, str(ii).zfill(5) + '.png'))
